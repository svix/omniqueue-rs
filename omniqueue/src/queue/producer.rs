...
pub trait QueueProducer: Send + Sync + Sized {
    ...
    fn into_dyn<'a>(self) -> DynProducer<'a>
    where
        Self: 'a,
    {
...
pub struct DynProducer<'a>(Box<dyn ErasedQueueProducer + 'a>);
...
impl<'a> crate::QueueProducer for DynProducer<'a> {
    type Payload = Vec<u8>;
